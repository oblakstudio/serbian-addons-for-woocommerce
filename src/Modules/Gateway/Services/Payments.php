...
class Payments {
    ...
    /**
     * Checks if the currency is RSD
     *
     * @param  ?string $currency Optional. Currency code. Default is the current currency.
     * @return bool
     */
    public function is_rsd( ?string $currency = null ): bool {
        $currency = \trim( \trim( $currency ?? \get_woocommerce_currency() ), '.' );
    ...
    private function format_currency( string $currency ): string {
        return $this->is_rsd( $currency ) ? 'RSD' : $currency;
    }
    ...
    public function format_address( WC_Order|WC_Customer|array $target, string $context = 'display' ): string {
        $this->config->set( 'address.formatting', true );
        ...
        $this->config->set( 'address.formatting', false );
